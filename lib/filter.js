...
function mermaid (type, value, _format, _meta) {
  ...
  const dimensions = getImageDimensions(savePath, options.format, options.scale)
  const imageAttrs = []
  if (dimensions) {
    imageAttrs.push(['width', `${dimensions.width}px`])
    imageAttrs.push(['height', `${dimensions.height}px`])
  }

  return pandoc.Figure(
    [id, imageClasses, imageAttrs],
...
function getImageDimensions (filePath, format, scale = 1) {
  if (format === 'png') {
    const buffer = fs.readFileSync(filePath)
    const width = buffer.readUInt32BE(16) / scale
    const height = buffer.readUInt32BE(20) / scale
    return { width, height }
  } else if (format === 'svg') {
